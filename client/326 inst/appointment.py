...
Driver: Emerson Kelley
Navigator: Stephanie Fishkin 
...
class Appointment: 
    ...
    def overlaps(self, other):
        #if #the start time of one appointment occurs between the start and end times of the other appointment (including if the start times are equal), OR
        #if #the end time of one appointment occurs between the start and end times of the other appointment (including if the end times are equal)

        if self.start <= other.start < other.end:
            return True 
...
def parse_args(args_list):
    ...
    #For example: #This function uses the argparse module in order to parse command line arguments.
